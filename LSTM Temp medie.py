...
#importazione dataframe originario
...
# Creazione del grafico originario
...
#Conversione della variabile 'Data' nel formato datetime
#e impostazione della data come indice
df['Data'] = pd.to_datetime(df['Data'])
df.set_index('Data', inplace=True)

#Standardizzazione della variabile "Gradi"
scaler = StandardScaler()
df['Gradi'] = scaler.fit_transform(df[['Gradi']])

#Suddivisione del dataframe in training e test set
...
#Costruzione e compilazione modello LSTM
model = Sequential()
...
#caricamento del modello LSTM
model = tf.keras.saving.load_model('model_LSTM.keras')

#stampa del modello LSTM
model.summary()
plot_model(model)

#Grafico funzione di loss train/test
...
test
# Creazione array di date (19-10-2020 / 31-12-2023)
...
#Grafico: confronto valori predetti (fitted) e valori reali (test set)
...
#inverione della conversione per riportare i dati ai valori originari
...
new_date = pd.date_range(start='2024-01-01', end='2027-02-12')
...
#nuovo datafame con valori originari e previsioni
df_new = pd.concat([df_original, predicted_grades_inverse_df], ignore_index=True)
df_new
df_original = pd.read_excel('df_temp_media_bis.xlsx')

# Creazione del grafico originario + previsioni
plt.figure(figsize=(12, 7))
